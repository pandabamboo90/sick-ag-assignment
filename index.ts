...
async function execute_session(connection: mqtt.MqttClientConnection, argv: Args) {
    return new Promise<void>(async (resolve, reject) => {
        try {
            si.cpuTemperature()
                .then((data: CpuTemperatureData) => {
                    const msg: any = {
                        deviceName: argv.device_name,
                        cpuTemp: data['main'],
                    }
                ...
                .then((json: string) => {
                    console.log(`--------------------------------------------`)
                    console.log(`Publishing data to topic "${argv.topic}" ...`);
                    console.log(`Payload: "${json}`);
                    return connection.publish(argv.topic, json, mqtt.QoS.AtLeastOnce)
                })
                .then(() => {
